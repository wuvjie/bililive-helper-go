...
| 层 | 技术 |
|----|------|
| 后端 | Go 1.23、Gin、go.uber.org/zap |
...
首次启动后，控制台会自动打印登录密码。登录后请在设置页面修改密码。
...
配置文件位于 `{LOG_DIR}/config.json`，首次初始化时自动生成。环境变量优先于配置文件。
...
| `GAP_MINUTES` | int | 场次间隔（分钟，超过此间隔视为新场次） | 60 |
