<template>
  <div class="setup-container">
    <div class="setup-card">
      ...
      <p class="setup-sub">首次启动配置</p>

      <el-form ref="formRef" :model="form" :rules="rules" @submit.prevent="handleSubmit" class="setup-form">
        <el-form-item prop="password">
          <label class="setup-label">设置登录密码</label>
          <el-input
            v-model="form.password"
            type="password"
            placeholder="至少 6 位，用于后续登录"
            show-password
          />
        </el-form-item>

        <el-form-item prop="target_dir">
          <label class="setup-label">录制视频目录</label>
          <el-input
            v-model="form.target_dir"
            placeholder="/vol2/1000/video/bililive-go/抖音"
            class="path-input"
          />
        </el-form-item>

        <el-form-item prop="log_dir">
          <label class="setup-label">日志和配置存储目录</label>
          <el-input
            v-model="form.log_dir"
            placeholder="/vol1/1000/docker/bililive-helper-go"
            class="path-input"
          />
        </el-form-item>
...
const rules = {
  ...
  target_dir: [{ required: true, message: "请填写录制目录", trigger: "blur" }],
  log_dir: [{ required: true, message: "请填写日志目录", trigger: "blur" }]
};
...
onMounted(async () => {
  try {
    ...
    if (status.log_dir) form.log_dir = status.log_dir;
  } catch {
...
/* Error message — muted rust red, no overlap */
.setup-form :deep(.el-form-item__error) {
  color: #c4554d;
  font-size: 12px;
  padding-top: 2px;
  position: static; /* Override absolute positioning to prevent overlap */
}
