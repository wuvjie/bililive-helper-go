...
const navItems = [
  ...
  },
  {
    path: '/streamers',
    name: '主播管理',
    svg: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 4.354a4 4 0 110 7.292 15.242 0 01-4.574 2.077M19 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75M9 7a4 4 0 11-8 0 4 4 0 018 0zM5 21v-2a4 4 0 014-4h6a4 4 0 014 4v2"/></svg>`
  },
  {
    path: '/tasks',
    name: '任务中心',
...
<template>
  <aside
    :class="[
      'h-screen bg-[#f7f8fa] border-r border-gray-200 flex flex-col transition-all duration-300 ease-in-out fixed left-0 top-0 z-40',
      sidebarWidth,
  ...
  >
    ...
    <div class="h-14 flex items-center px-4 border-b border-gray-200">
      ...
      <Transition name="fade">
        <span v-if="appStore.sidebarExpanded" class="ml-3 text-gray-900 font-semibold text-sm whitespace-nowrap">
          Bililive Helper
        </span>
      </Transition>
    </div>
        ...
        >
          <div
            ...
            :class="[
              ...
              isActive(item.path) ? 'text-[#3370ff]' : 'text-gray-500 group-hover:text-gray-700'
            ]"
          ></div>
          <Transition name="fade">
            <span
              ...
              :class="[
                ...
                isActive(item.path) ? 'text-[#3370ff]' : 'text-gray-600 group-hover:text-gray-900'
              ]"
            >
              {{ item.name }}
            </span>
          ...
          <div
            ...
            class="absolute left-0 top-1/2 transform -translate-y-1/2 w-[3px] h-5 bg-[#3370ff] rounded-r"
          ></div>
        </button>
      </div>
    </nav>
    ...
    <div class="p-2 border-t border-gray-200">
      <button
        ...
        class="w-full flex items-center py-2 px-3 rounded-lg text-gray-500 hover:bg-gray-100 hover:text-gray-700 transition-all"
      >
        <svg
          :class="[
        ...
        <Transition name="fade">
          <span v-if="appStore.sidebarExpanded" class="ml-2 text-sm whitespace-nowrap">
            收起导航
          </span>
        </Transition>
      </button>
