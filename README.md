...
│   │   ├── config.go              # 配置读写、推荐、导入导出、内容分析
│   │   ├── handler.go             # Handler 结构体定义与构造函数
│   │   ├── history.go             # 历史记录分页查询、操作日志查看（log_id）
...
│   │   ├── proc_unix.go           # Unix 进程组管理（setpgid、kill）
│   │   ├── proc_windows.go        # Windows 进程管理兼容层
...
│       ├── disk_windows.go        # 磁盘用量查询 Windows 兼容层
...
日志存储在 `{LOG_DIR}/{task}_log/` 目录下，每天自动轮转，保留 30 天。每次手动或自动执行的合并/清理操作还会创建独立的操作日志文件 `op_{logID}.log`，可通过操作日志页面查看详细输出。
